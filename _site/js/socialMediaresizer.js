...
root.style.setProperty('--mainUIColorSecondary',LightenDarkenColor(UIData[currentURL]["mainColor"].slice(1),10));

let topDiv=document.querySelector('.workspace .topDiv')
topDiv.style.background=UIData[currentURL]["mainColor"];


let Title=document.querySelector('.title_logo .Title');
Title.innerText=`Resize image for ${currentURL[0].toLowerCase()+currentURL.slice(1)}`;

let LandingIcon=document.querySelector('.LandingIcon');
LandingIcon.src=UIData[currentURL]['iconURL'].slice(0,26)+'iconsForLanding/'+UIData[currentURL]['iconURL'].slice(26);
...
let boxContainer=document.querySelector(".innerContent");
...
let downloadCurrentSize=(e)=>{
    ...
    {   
        await resizer.toBlob(canvasTemp)
        .then(async blob=>{
...
let showPreviews=()=>{
    ...
    {   
        ...
        image.style.height=300*currentData[size][1]/currentData[size][0]+'px';
...
let triggerDownload=()=>{
    ...
    document.querySelector(".Landing .Container h3").innerText="thanks for your patience";
    document.querySelector(".Landing .Container .ProgressInfo img").src="./img/imageResizer-assets/cheers.svg";
    zip.generateAsync({type:"blob"}).then(function(content) {saveAs(content, `ImageResizer.zip`);});
}
...
let handleDownload=(e)=>{
    ...
    document.querySelector(".Landing .Container").classList.add("ContainerOnloading");
