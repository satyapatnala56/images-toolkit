...
let cropRatios = document.querySelectorAll('.crop-aspect-ratio')
let drawingShapes = document.querySelectorAll('.drawing-shape')
let selectTextPosition = document.querySelectorAll('.select-text-position')
...
const handleFile = (inputFile) => {
  ...
  imageEditor.loadImageFromFile(file).then(function (result) {
    ...
    fileDropBox.style.display = 'none'
    document.querySelector('#btn-crop').click()
...
var $btnUndo1 = $('#btn-undo1')
var $btnRedo1 = $('#btn-redo1')
...
const textFontValue = document.querySelector('#text-font')
// Sub menus
...
var brushColorpicker = document.querySelector('#tui-brush-color-picker')

// Color picker for text palette
var textColorpicker = document.querySelector('#text-color-picker')

// Color picker for shape
var shapeStrokeColorpicker = document.querySelector('#stroke-color-picker')
var shapeFillColorpicker = document.querySelector('#fill-color-picker')
...
var tintColorpicker = document.querySelector('#tui-tint-color-picker')

var multiplyColorpicker = document.querySelector('#tui-multiply-color-picker')

var blendColorpicker = document.querySelector('#tui-blend-color-picker')
...
function getBrushSettings() {
  ...
  var brushColor = brushColorpicker.value
...
function setShapeToolbar(obj) {
  ...
  if (colorType === 'stroke') {
    if (!isTransparent && !isFilter) {
      shapeStrokeColorpicker.value = changeValue
    }
  } else if (colorType === 'fill') {
    if (!isTransparent && !isFilter) {
      fillColor = obj.fill.color
      shapeFillColorpicker.value = fillColor
    }
  }
...
imageEditor.on({
  ...
  undoStackChanged: function (length) {
    if (length) {
      ...
      $btnUndo1.removeClass('disabled')
    } else {
      $btnUndo.addClass('disabled')
      $btnUndo1.addClass('disabled')
    }
    resizeEditor()
  },
  redoStackChanged: function (length) {
    if (length) {
      ...
      $btnRedo1.removeClass('disabled')
    } else {
      $btnRedo.addClass('disabled')
      $btnRedo1.addClass('disabled')
    }
    resizeEditor()
  },
...
Array.from(drawingShapes).map((i) => {
  i.addEventListener('click', (e) => {
    Array.from(drawingShapes).map((item) => {
...
Array.from(cropRatios).map((item) => {
  item.addEventListener('click', (e) => {
    ...
    switch (e.currentTarget.dataset.ratio) {
      ...
      case '5/4':
        ...
        break
      case '7/5':
        imageEditor.setCropzoneRect(7 / 5)
        break
      case '16/9':
...
$btnRotateClockwise.on('click', function () {
  if (parseInt($rotateValue.html()) < 360) {
    let angleValue = 30
    if (parseInt($rotateValue.html()) + 30 > 360) {
      angleValue = 360 - parseInt($rotateValue.html())
    }
    imageEditor.rotate(angleValue)
    $inputRotationRange.val(Number($rotateValue.html()) + angleValue)
    $rotateValue.html(Number($rotateValue.html()) + angleValue)
  }
})
...
$inputRotationRange.on('mousedown', function () {
  $rotateValue.html(parseInt($inputRotationRange.val(), 10))
...
$inputRotationRange.on('input', function () {
  $rotateValue.html(parseInt($inputRotationRange.val(), 10))
  imageEditor
    .setAngle(parseInt($inputRotationRange.val(), 10))
    ['catch'](function () {})
...
$inputBrushWidthRange.on('change', function () {
  document.querySelector('#brush-width-value').innerHTML = parseInt(
    this.value,
    10
  )
...
$selectLine.on('change', function () {
  ...
  imageEditor.stopDrawingMode()
  if (mode === 'freeDrawing') {
    document.querySelector('#free-mode').classList.add('active-option')
    document.querySelector('#line-mode').classList.remove('active-option')
    imageEditor.startDrawingMode('FREE_DRAWING', settings)
  } else {
    document.querySelector('#free-mode').classList.remove('active-option')
    document.querySelector('#line-mode').classList.add('active-option')
    imageEditor.startDrawingMode('LINE_DRAWING', settings)
  }
})
brushColorpicker.addEventListener('input', (event) => {
  imageEditor.setBrush({
    color: hexToRGBa(event.target.value, 0.5),
  })
})
...
shapeFillColorpicker.addEventListener('input', (event) => {
  $inputCheckTransparent.prop('checked', false)
  $inputCheckFilter.prop('checked', false)
  onChangeShapeFill(event)
})
shapeStrokeColorpicker.addEventListener('input', (event) => {
  $inputCheckTransparent.prop('checked', false)
  $inputCheckFilter.prop('checked', false)
  onChangeShapeFill(event)
})
...
function onChangeShapeFill(event) {
  var colorType = event.target.dataset.colortype
  var shapeOption
  ...
  if (event.target.value) {
    if (event.target.value === 'filter') {
      ...
      shapeOption = event.target.value
    }
  }
  if (colorType === 'stroke') {
...
$inputStrokeWidthRange.on('change', function () {
  document.querySelector('#stroke-width').innerHTML = Number($(this).val())
...
$inputFontSizeRange.on('change', function () {
  textFontValue.innerHTML = parseInt(this.value, 10)
...
$btnTextStyle.on('click', function (e) {
  ...
  switch (styleType) {
    ...
    case 'l':
      ...
      Array.from(selectTextPosition).map((i) => {
  ...
  let findClass = e.currentTarget.classList.value.includes('active-option')
  if (findClass) {
    e.currentTarget.classList.remove('active-option')
  } else {
    e.currentTarget.classList.add('active-option')
  }
...
$inputCheckMultiply.on('change', function () {
  applyOrRemoveFilter(this.checked, 'blendColor', {
    color: multiplyColorpicker.value,
  })
})

multiplyColorpicker.addEventListener('input', (e) => {
...
$inputCheckBlend.on('change', function () {
  applyOrRemoveFilter(this.checked, 'blendColor', {
    ...
    color: blendColorpicker.value,
  })
})

blendColorpicker.addEventListener('input', (e) => {
...
// Load sample image
imageEditor
  .loadImageFromURL('/img/editor.png', 'SampleImage')
