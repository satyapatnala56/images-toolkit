...
const handleDownload = () => {
  ...
  console.log(cropInputHeight.value)
  console.log(cropInputWidth.value)
  let imgCanvas = cropper.getCroppedCanvas({
    width: cropInputHeight.value,
    height: cropInputWidth.value,
  })
  if (imgData === '') {
    ...
    img.onload = () => {
      let img2 = new Image()
      let img3 = new Image()
      img3.src = imgCanvas.toDataURL()
      console.log(fileType)
      img3.onload = () => {
        if (fileType === 'png') {
          let pngCanvas = document.createElement('canvas')
          let pngCtx = pngCanvas.getContext('2d')
          pngCanvas.width = cropInputWidth.value
          pngCanvas.height = cropInputHeight.value
          pngCtx.fillStyle = '#ffffff'
          pngCtx.fillRect(0, 0, img3.width, img3.height)
          pngCtx.drawImage(
            img3,
            0,
            0,
            cropInputWidth.value,
            cropInputHeight.value
          )
          img2.src = pngCanvas.toDataURL()
        } else {
...
const rotate = (e) => {
  cropper.rotateTo(Number(e.value))
  document.querySelector('#rotatevalue').innerHTML =
    Number(e.value) + ' ' + 'deg'
}
